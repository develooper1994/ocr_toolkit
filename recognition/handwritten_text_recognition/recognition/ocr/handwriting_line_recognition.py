...
if __name__ == "__main__":
    ...
    net = Network(num_downsamples=num_downsamples, resnet_layer_id=resnet_layer_id, rnn_hidden_states=rnn_hidden_states,
    ...
    test_data = gluon.data.DataLoader(test_ds.transform(transform), batch_size, shuffle=True, last_batch="discard",
                                      num_workers=4 * len(ctx))
    ...
    trainer = gluon.Trainer(net.collect_params(), 'adam',
                            {'learning_rate': learning_rate, "lr_scheduler": schedule, 'clip_gradient': 2})
